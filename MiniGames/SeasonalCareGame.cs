    ...
    public class SeasonalCareGame : MiniGameBase
    {
        ...
        private void GenerateRandomWeatherCondition()
        {
            ...
            switch (currentSeason)
            {
                case Season.Winter:
                    {
                        int[] possibleWeathers = { (int)Weather.Sunny, (int)Weather.Cloudy, (int)Weather.Snow, (int)Weather.Wind };
                        int index = random.Next(possibleWeathers.Length);
                        currentWeather = (Weather)possibleWeathers[index];
                        break;
                    }
                case Season.Spring:
                    {
                        int[] possibleWeathers = { (int)Weather.Sunny, (int)Weather.Cloudy, (int)Weather.Rain, (int)Weather.Wind };
                        int index = random.Next(possibleWeathers.Length);
                        currentWeather = (Weather)possibleWeathers[index];
                        break;
                    }
        ...
        private void UpdateUI()
        {
            ...
            foreach (Control control in gamePanel.Controls)
            {
                if (control is Panel panel)
                {
                    ...
                    {
                        if (panelControl is Label label && label.Name == "turnLabel")
                        {
                            label.Text = $"Turn {careTurns}/{maxTurns}";
                        }
                        else if (panelControl is Label scoreLabel && scoreLabel.Name == "scoreLabel")
                        {
        ...
        private void EndGame()
        {
            ...
            // Show final score
            MessageBox.Show(
                $"Game Over!\n\nYour final score: {(int)normalizedScore}/100\n\n{resultMessage}",
                "Game Complete",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information);
